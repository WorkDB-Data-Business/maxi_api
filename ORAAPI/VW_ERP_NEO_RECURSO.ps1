...
$tabelas = @("TB_ERP_NEO_RECURSO")  # Substitua pelos nomes das suas tabelas
...
foreach ($tabela in $tabelas) {
    ...
    if ($existe -eq 1) {
        ...
        $command.CommandText = "CREATE TABLE $tabela (UNIDADE_FABRIL NVARCHAR(80), ID_RECURSO NVARCHAR(43), CODIGO_RECURSO NVARCHAR(8), DESCRE NVARCHAR(40), PEREFI INT )" 
        $command.ExecuteNonQuery()
