...
$tabelas = @("TB_ERP_NEO_APONTAMENTO")  # Substitua pelos nomes das suas tabelas
...
foreach ($tabela in $tabelas) {
    ...
    if ($existe -eq 1) {
        ...
        $command.CommandText = "CREATE TABLE $tabela (UNIDADE_FABRIL NVARCHAR(80), NUMORP NVARCHAR(43), CODOPR NVARCHAR(6), QTD_APONTADA INT, DATINI DATE , DATREA DATE, CODCRE NVARCHAR(8), HORINI NVARCHAR(5), HORREA NVARCHAR(5))"  # Substitua pelas colunas necessárias
        $command.ExecuteNonQuery()
