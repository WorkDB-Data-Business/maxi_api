...
$tabelas = @("TB_ERP_NEO_UNIDADE_FABRIL")  # Substitua pelos nomes das suas tabelas
...
foreach ($tabela in $tabelas) {
    ...
    if ($existe -eq 1) {
        ...
        $command.CommandText = "CREATE TABLE $tabela (UNIDADE_FABRIL NVARCHAR(80), NOMFIL NVARCHAR(100), DESCRICAO NVARCHAR(30))" 
        $command.ExecuteNonQuery()
