...
$tabelas = @("TB_ERP_NEO_ATRIBUTOS")  # Substitua pelos nomes das suas tabelas
...
foreach ($tabela in $tabelas) {
    ...
    if ($existe -eq 1) {
        ...
        $command.CommandText = "CREATE TABLE $tabela (CODMOD NVARCHAR(14), CODREG INT, NOMEATB NVARCHAR(250), ATRIBUTO_TEXTO NVARCHAR(1500), ATRIBUTO_NUMERO  REAL, UNIMED NVARCHAR(3), ITEM NVARCHAR(50) )" 
        $command.ExecuteNonQuery()
