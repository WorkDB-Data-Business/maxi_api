...
$tabelas = @("TB_ERP_NEO_ITEM")  # Substitua pelos nomes das suas tabelas
...
foreach ($tabela in $tabelas) {
    ...
    if ($existe -eq 1) {
        ...
        $command.CommandText = "CREATE TABLE $tabela (UNIDADE_FABRIL NVARCHAR(80), ID_ITEM	NVARCHAR(21), CODIGO_ITEM  NVARCHAR(21), DESCRICAO	NVARCHAR(151), TIPPRO	NVARCHAR(1), LARDER	REAL, USU_ESPPRO	INT, PIGMENTO	NVARCHAR(100), OUTROS_ATRIBUTOS	VARCHAR(500), CODIGO_REFERENCIA	VARCHAR(40), CODMOD	NVARCHAR(20), ATRIBUTO	INT,  NOME_ATRIBUTO	NVARCHAR(250), ITEM_ATRIBUTO	NVARCHAR(50))" 
        $command.ExecuteNonQuery()
