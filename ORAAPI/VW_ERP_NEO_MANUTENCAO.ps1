...
$tabelas = @("TB_ERP_NEO_MANUTENCAO")  # Substitua pelos nomes das suas tabelas
...
foreach ($tabela in $tabelas) {
    ...
    if ($existe -eq 1) {
        ...
        $command.CommandText = "CREATE TABLE $tabela (UNIDADE_FABRIL NVARCHAR (80), ORDEM_MANUTENCAO INT, RECURSO NVARCHAR(20), TIPO_MANUTENCAO NVARCHAR(20), FINAL_MANUTENCAO NVARCHAR(20), DESMNT NVARCHAR(1999))" 
        $command.ExecuteNonQuery()
