...
$tabelas = @("TB_ERP_NEO_ESTOQUE")  # Substitua pelos nomes das suas tabelas
...
foreach ($tabela in $tabelas) {
    ...
    if ($existe -eq 1) {
        ...
        $command.CommandText = "CREATE TABLE $tabela (UNIDADE_FABRIL NVARCHAR(80), ID_ITEM NVARCHAR(21), CODDEP NVARCHAR(10), QTD_ESTOQUE INT, QTD_RESERVADO INT , QTD_BLOQUEADO INT, QTD_LIQUIDO INT, UNIMED NVARCHAR(3))"  # Substitua pelas colunas necessárias
        $command.ExecuteNonQuery()
