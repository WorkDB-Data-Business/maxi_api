...
$tabelas = @("TB_ERP_NEO_GRUPO_RECURSO")  # Substitua pelos nomes das suas tabelas
...
foreach ($tabela in $tabelas) {
    ...
    if ($existe -eq 1) {
        ...
        $command.CommandText = "CREATE TABLE $tabela (UNIDADE_FABRIL NVARCHAR(80), GRUPO NVARCHAR(9), DESCRICAO NVARCHAR(80), MAQUINA NVARCHAR(8))"  # Substitua pelas colunas necessárias
        $command.ExecuteNonQuery()
