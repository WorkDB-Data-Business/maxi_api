...
$tabelas = @("TB_ERP_NEO_ORDEM_COMPRA")  # Substitua pelos nomes das suas tabelas
...
foreach ($tabela in $tabelas) {
    ...
    if ($existe -eq 1) {
        ...
        $command.CommandText = "CREATE TABLE $tabela (UNIDADE_FABRIL NVARCHAR(80), NUMOCP INT, ID_ITEM NVARCHAR(21), DATENT DATE, QTDREC INT , UNIMED NVARCHAR(3))"  # Substitua pelas colunas necessárias
        $command.ExecuteNonQuery()
