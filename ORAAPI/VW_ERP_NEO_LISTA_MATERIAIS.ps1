...
$tabelas = @("TB_ERP_NEO_LISTA_MATERIAIS")  # Substitua pelos nomes das suas tabelas
...
foreach ($tabela in $tabelas) {
    ...
    if ($existe -eq 1) {
        ...
        $command.CommandText = "CREATE TABLE $tabela (UNIDADE_FABRIL NVARCHAR (80), NUMORP NVARCHAR(43), SEQROT INT, ITEM_REQUISITADO NVARCHAR(212), QTDPRV REAL, UNIDADE_PAI  NVARCHAR(3), UNIDADE_FILHO NVARCHAR(3), ITEM_PAI NVARCHAR(21), NUMPED INT )" 
        $command.ExecuteNonQuery()
